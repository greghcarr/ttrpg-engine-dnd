# Changelog archive: slices 252-260

These entries were originally in the `## Unreleased` section of [../../CHANGELOG.md](../../CHANGELOG.md). The slice-by-slice detail was moved here in slice 270 to keep the live CHANGELOG under the single-Read ceiling, following the same archive pattern as the [sibling archives](.) for older slices.

This window covers the post-alpha.6 polish cluster + the audit-gap-fix trio + the closure-annotation conventions:

- Slice 252: alpha.6 slice detail archived + archive-path correctness sweep (207 broken-link fix across 14 archive files)
- Slice 253: variable `chargesCost` on `CastSpell` UseAction + Wand of Magic Missiles canonical user
- Slice 254: feature-coverage matrix counts `onUse` wires as wired
- Slice 255: content sweep (Wand of Fireballs + Wand of Lightning Bolts + Staff of Healing Cure Wounds arm)
- Slice 256: per-item degradation roll primitive + Wand of Magic Missiles canonical user
- Slice 257: content sweep (degradation roll wired on the remaining wands + Staff of Healing)
- Slice 258: SetAdvantage.condition honored + `event.isSpellSave` + Mantle of Spell Resistance
- Slice 259: `inventory` + `attunedInstanceIds` options on buildFighter / buildOgre fixtures
- Slice 260: closure annotations on past CHANGELOG `Open follow-ups` blocks (introduces the `~~strikethrough~~` + `**Closed by slice N.**` convention)

Order: most-recent first (slice 260 at top, slice 252 at bottom). For more recent unreleased slices (261+), see the live [CHANGELOG.md](../../CHANGELOG.md). For older archived slices, see the sibling archives in this directory (indexed from the live CHANGELOG's archive pointer block).
