# Changelog archive: slices 241-250 (alpha.6 release block)

These entries were originally in the `## Unreleased` section of [../../CHANGELOG.md](../../CHANGELOG.md), then promoted to `## 0.1.0-alpha.6 - 2026-05-18` in slice 251. The slice-by-slice detail was moved here in slice 252 to keep the live CHANGELOG under the single-Read ceiling, following the same archive pattern as the [sibling archives](.) for older slices.

This window covers the post-rename + fresh-agent-test cluster:

- Slice 241: `CastSpell` UseAction + at-will spell-grant items
- Slice 242: `Toggle` UseAction + Boots of Speed
- Slice 243: action-selector + per-action chargesCost + Staff of Healing
- Slice 244: onboarding refresh + dev-branch convention
- Slice 245: SRD 5.2.1 markdown as git submodule
- Slice 246: fresh-agent discovery polish
- Slice 247: package + repo rename to `dnd-srd-engine`
- Slice 248: single-Read ceiling fits across front-door docs
- Slice 249: `starter-pack-gaps` split into per-category catalogs
- Slice 250: README top-level-dir map

Order: most-recent first (slice 250 at top, slice 241 at bottom). For more recent unreleased slices (251+), see the live [CHANGELOG.md](../../CHANGELOG.md). For older archived slices, see the sibling archives in this directory (indexed from the live CHANGELOG's archive pointer block).
