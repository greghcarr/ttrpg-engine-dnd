...
**Secondary monster fields (slice 154, audit only).** Follow-up to the slice 141 to 149 primary monster audit. Audit at [docs/srd-5.2.1-audit-monsters-secondary.md](srd-5.2.1-audit-monsters-secondary.md). Covers size / type / alignment / speed / saving throws / skills / senses / damage resistances + immunities + vulnerabilities / condition immunities; does not cover action / trait text bodies. 10 of 111 monsters are clean on secondary fields; 101 have at least one diff (270+ field-level diffs total). Two schema-rename bugs (Wolf + Young Red Dragon use `passivePerception` instead of `passivePerceptionOverride`); the rest is drift between 2014 MM secondary stats and SRD 5.2.1. Fixes queued for slices 155 to 164.

**Classes (slice 153, completed).** Audit at [docs/srd-5.2.1-audit-classes.md](srd-5.2.1-audit-classes.md). All 12 class names and all 12 subclass names match SRD 5.2.1 exactly. Main-class feature catalog has 13 level-placement drift entries (e.g. Fighter Tactical Mind at pack L5 vs SRD L2; Rogue Reliable Talent at pack L11 vs SRD L7) and ~17 SRD-listed features genuinely missing from pack (e.g. Cleric Divine Order at L1; Monk Heightened Focus at L10). Subclass feature catalog has the largest content gap: 41 SRD-listed subclass features missing from pack (most subclasses ship only L3 features; SRD adds 3-4 more per subclass at higher levels). Five PHB 2014-flavored pack features (Empty Body, Pact Boon, Improved Divine Intervention, Step of the Wind: Heightened Mobility, Sorcery Points at L20) don't appear in SRD 5.2.1 and are kept under the same hybrid policy as slice 152 (PHB extras stay; downstream consumers can prune). Fixes are content-authoring follow-ups; slice 153 is documentation-only.
