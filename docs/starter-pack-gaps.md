...
| Spells | 399 / 399 | ~399 | 34 / 60 / 63 / 54 / 40 / 46 / 38 / 24 / 19 / 21 across L0–L9. **Every PHB 2024 spell now ships in the pack.** ~139 wired with `mechanicalEffects` (~18 cantrips, ~30 L1, ~23 L2, ~19 L3, ~12 L4, ~13 L5, ~10 L6, ~5 L7, ~6 L8, ~3 L9) + 7 dedicated planners (counterspell, dispel-magic, identify, misty-step, shield, hunters-mark, polymorph). The remaining ~261 ship schema-only, each blocked on a named engine primitive captured in the per-level sections below. |
...
**Schema-only (34):** grouped by the engine primitive each one needs.
...
- **Caster-chosen options at cast time** (damage type chosen): `elemental-weapon` (damage type + needs item-buff damage-type extension since it touches a specific weapon, not the wielder). (Spirit Shroud wired in slice 89.)
