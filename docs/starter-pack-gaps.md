...
| **~~Type-conditional buff / ward~~** ✓ shipped (partial) | 3 wired (protection-from-evil-and-good, magic-circle, holy-aura). The source-gated `GrantConditionImmunity` arm of PfEoG (charmed / frightened only from the six types) still ships narrative-only; a future slice can add a source-predicate to GrantConditionImmunity once a second user appears. | Slice 103 extended `ImposeDisadvantageOnAttackers` to honor its existing `condition?: Predicate` field: the EffectAccumulator stores per-entry predicates, and `imposesDisadvantageOnAttackers(attackerFacts)` filters them against caller-supplied facts. The attack planner now passes `attackerCreatureType` (resolved via `getCreatureType(attacker, content)` — monster.type via statblockId for creatures, species.creatureType for PCs / NPCs, default Humanoid). Slice 103 also added an `event.attackerCreatureType` fact in `dispatchTriggers.buildEventFacts` and a new `ApplyConditionToAttacker` TriggerAction variant that retargets the emitted ConditionApplied to the attacker (the AddDamageToAttacker shape applied to conditions). Holy Aura uses both — universal ImposeDisadvantageOnAttackers + universal save-advantage on the condition's effects, plus an OnEvent rider with an `event.attackerCreatureType in ['Fiend', 'Undead']` filter emitting `ApplyConditionToAttacker { blinded }`. PfEoG and Magic Circle share an `any`-of-six-types predicate on ImposeDisadvantageOnAttackers. Magic Circle's RAW caster-choice-of-type isn't modeled (broader than RAW: the ward applies against all six types regardless of caster intent). |
