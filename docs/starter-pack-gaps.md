...
**Schema-only (40):** grouped by the engine primitive each one needs.
...
| **~~AddDamageToAttacker TriggerAction~~** ✓ shipped | 1 wired (fire-shield, both warm/chill variants). Armor of Agathys uses the same shape but is gated on "while temp HP > 0" — needs a bearer-state predicate (consult character state from the dispatch predicate) before it can wire, and the spell isn't in the pack today. | Slice 100 adds the `AddDamageToAttacker` TriggerAction variant of the existing AddDamage shape (same dice + damageType fields); the dispatcher's new `fireAddDamageToAttacker` helper builds a DamageApplied targeting `event.attackerId` instead of `event.targetId`. Crits on the triggering attack don't double the retaliation dice per RAW. Only fires against AttackRolled triggers. |
