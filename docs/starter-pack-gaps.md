...
| Spells | 399 / 399 | ~399 | 34 / 60 / 63 / 54 / 40 / 46 / 38 / 24 / 19 / 21 across L0–L9. **Every PHB 2024 spell now ships in the pack.** ~134 wired with `mechanicalEffects` (~18 cantrips, ~28 L1, ~22 L2, ~17 L3, ~12 L4, ~13 L5, ~10 L6, ~5 L7, ~6 L8, ~3 L9) + 7 dedicated planners (counterspell, dispel-magic, identify, misty-step, shield, hunters-mark, polymorph). The remaining ~266 ship schema-only, each blocked on a named engine primitive captured in the per-level sections below. |
...
- **Caster-chosen options at cast time** (per-word effect variants): `command`. (Chromatic Orb's caster-chosen damage type now wires via slice 82's `casterChoosesDamageType` field on attack mechanics.)
...
**Wired (22):** aid, blindness-deafness, branding-smite (on-hit rider, one-shot), calm-emotions (caster-chosen save variant), crown-of-madness, enlarge-reduce (caster-chosen buff variant), find-steed (summon), flame-blade, heat-metal, hold-person, invisibility, lesser-restoration, melfs-acid-arrow, moonbeam, prayer-of-healing, protection-from-poison, scorching-ray, shatter, spiritual-weapon, suggestion, summon-beast (summon), web.

**Dedicated planner (1):** misty-step.

**Schema-only (41):** grouped by the engine primitive each one needs.
...
- **Caster-chosen options at cast time** (variant chosen by caster, ability target chosen): `enhance-ability`. (`enlarge-reduce` wired in slice 83 via the buff mechanic's `casterChoosesVariant` field; `calm-emotions` wired in slice 84 via the same shape on the save mechanic. RAW edge: the suppress variant's "end existing Charmed/Frightened" half isn't auto-applied — the immunity grant blocks future application, not existing instances; consumers can emit ConditionRemoved if desired. The on-hit ±1d4 weapon-damage rider for Enlarge/Reduce stays deferred — needs a per-attack damage-die rider that isn't yet modeled.)
...
| **~~Caster-chosen options at cast time~~** ✓ shipped (partial) | 3 wired (chromatic-orb: caster-chosen damage type on attack mechanics; enlarge-reduce: caster-chosen variant on buff mechanics; calm-emotions: caster-chosen variant on save mechanics). ~5 still deferred: command (per-word effects), enhance-ability (Bear/Bull/Cat/Eagle/Fox/Owl), bestow-curse (one of four curse variants), elemental-weapon (damage type for weapon enhance + needs the item-buff damage-type extension), spirit-shroud (damage type for on-hit rider + needs the on-hit primitive extension). | Slice 82 introduced the `CasterChoice` discriminated-union on `CastSpellIntent` plus `casterChoosesDamageType: { allowed: DamageType[] }` on attack mechanics. Slice 83 added the `'variant'` discriminator case (`{ kind: 'variant', value: string }`) plus `casterChoosesVariant: { variants: [{ key, conditionId }] }` on buff mechanics. Slice 84 ported the same `casterChoosesVariant` shape to save mechanics — the variant key picks the condition that applies on a failed save. The buff and save resolvers share a `resolveVariantConditionId` helper. Same planner validation pattern: throws on missing / wrong-kind / unknown-key choices. The next variants to land are `'ability'` (enhance-ability picks STR / DEX / etc; needs an ability-check-buff condition with the chosen ability folded in) and a multi-variant buff (bestow-curse's pool of one-of-four curses fits the existing `casterChoosesVariant` shape but needs four cursed conditions authored). Distinct from the `PendingChoice` protocol, which models deferred decisions persisted in state (level-up choices). |
