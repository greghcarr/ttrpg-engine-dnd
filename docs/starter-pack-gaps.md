...
| Spells | 399 / 399 | ~399 | 34 / 60 / 63 / 54 / 40 / 46 / 38 / 24 / 19 / 21 across L0–L9. **Every PHB 2024 spell now ships in the pack.** ~139 wired with `mechanicalEffects` (~18 cantrips, ~30 L1, ~23 L2, ~19 L3, ~12 L4, ~13 L5, ~10 L6, ~5 L7, ~6 L8, ~3 L9) + 8 dedicated planners (counterspell, dispel-magic, elemental-weapon, identify, misty-step, shield, hunters-mark, polymorph). The remaining ~260 ship schema-only, each blocked on a named engine primitive captured in the per-level sections below. |
...
**Schema-only (31):** grouped by the engine primitive each one needs.
...
- **Reaction system** (cast as a reaction to a trigger event): `absorb-elements`, `sanctuary`. Also future Silvery Barbs and a Shield-as-spell variant. (Feather Fall slipped this bucket by modeling its protection as a passive effect that `planFalling` consults; the generic reaction-window primitive is still unbuilt.)
...
| **Reaction system** | ~2 (absorb-elements, sanctuary), plus all future reaction spells | Cast as a reaction to a trigger event; needs `ReactionRegistered` event + a window on the appropriate `apply` calls. Feather Fall slipped this bucket via slice 81: rather than build the generic reaction-window primitive, the spell was modeled as a `buff` that applies `feather-falling-active` (carrying a new `GrantFallingProtection` effect kind), and `planFalling` short-circuits to no events when the falling character has any `GrantFallingProtection` in its effect stack. Absorb Elements (rider on incoming damage) and Sanctuary (rider on outgoing attacks against the protected target) still need the real reaction system. |
