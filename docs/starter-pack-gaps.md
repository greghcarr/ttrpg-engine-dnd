...
| **~~Area-effect spell mechanic~~** ✓ shipped (partial) | 9 wired (stinking-cloud, wall-of-fire, wall-of-thorns, wall-of-ice, blade-barrier, entangle, grease, black-tentacles, flaming-sphere) | Slice 68 extends the `aura-damage` mechanic: `damageDice` and `damageType` become optional (so condition-only zones like Stinking Cloud / Entangle / Grease work), and a new `conditionOnFail` field applies a condition to creatures that fail the save (gated by `isImmuneToCondition`, so Aura of Courage etc. block Frightened-style projections cleanly). `planTickAura` handles damage-only, condition-only, and combined paths. Spells fire via `engine.plan.tickAura` — the consumer is responsible for tracking "creature entered the zone" or "creature started its turn in it" and calling tickAura with that target. Slice 69 wired entangle (STR → restrained), grease (DEX → prone), black-tentacles (DEX 3d6 bludgeoning + restrained), and flaming-sphere (DEX 2d6 fire) on the same extended primitive. Remaining area-effect spells (cloud-of-daggers, spike-growth, hunger-of-hadar, plant-growth, slow, dust-devil, silence, zone-of-truth, darkness, wind-wall, leomunds-tiny-hut) still need additional primitives: auto-damage without save (Cloud of Daggers), damage-per-foot-moved (Spike Growth), pure terrain marker (Plant Growth), recurring-rider for damage-per-turn-in-zone (Hunger of Hadar), and the area-of-pure-obscurement / silence zones. |
