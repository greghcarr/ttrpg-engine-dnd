...
- **Reaction system** (cast as a reaction to a trigger event): `sanctuary`. Also future Silvery Barbs. Absorb Elements wired in slice 91 as a dedicated reaction planner (mirrors the slice-29 Shield pattern): caller passes the triggering DamageApplied's id + type + amount; the planner emits a compensating Healed event for the absorbed half + a per-type `absorb-elements-charged-X-active` condition that rides the caster's next melee hit (consumeOnTrigger). Sanctuary still needs a pre-attack target-selection rider that fires on a hypothetical `AttackTargeted` event before the d20 rolls — different shape entirely. Feather Fall slipped this bucket via slice 81's passive `GrantFallingProtection` effect on `planFalling`.
...
| **~~Reaction system~~** ✓ shipped (partial) | 4 wired across slices (counterspell, shield, absorb-elements, plus feather-fall via passive-effect side door). Sanctuary still needs a pre-attack target-selection rider; future spells like Silvery Barbs and a Shield-as-spell variant will reuse the same dedicated-reaction-planner pattern. | The engine doesn't have a generic `ReactionRegistered` event + `apply`-side reaction window. Instead each reaction spell ships as a dedicated planner that the consumer calls after seeing the triggering event: planShield (after AttackRolled), planCounterspell (after a SpellCastDeclared from another caster), planConsumeGuidance (when the bearer wants to spend their d4), and now planAbsorbElements (after a DamageApplied of a matching element). All four go through `assertReactionAvailable` + `economyConsumedIfEncountered`. AbsorbElements specifically uses an event-sourcing-friendly approach: rather than mutate the triggering DamageApplied, it emits a compensating Healed event for the absorbed half — and the per-type `absorb-elements-charged-X-active` condition (acid / cold / fire / lightning / thunder) carries the on-next-hit rider via `OnEvent + AddDamage + consumeOnTrigger`. Slot-level scaling on the rider (RAW: +1d6 per slot above 1st) isn't modeled — the dice are hardcoded at 1d6. Sanctuary (pre-attack target-selection save) and Silvery Barbs (reroll a d20 from a different creature) each need their own dedicated planner; the slice-91 pattern keeps the door open without a generic reaction window. Feather Fall slipped this bucket entirely via slice 81's passive `GrantFallingProtection` on `planFalling`. |
