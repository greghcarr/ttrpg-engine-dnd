...
| **~~Movement-mode condition~~** ✓ shipped (content) | 2 wired (fly, spider-climb). Levitate still deferred — RAW is a vertical-only positional effect, not a movement-mode addition. | The `ModifySpeed` effect kind already supports `mode: 'fly' \| 'climb'`. Fly + Spider Climb wire as buff conditions carrying ModifySpeed entries. Note: the engine's `getEffectiveSpeed` planner currently reads `character.speedFeet` directly without walking the effect stack — same pre-existing gap that means Fast Movement / Unarmored Movement / Roving don't yet bump the character's actual move-distance either. Content ships as discoverable metadata so consumers (dndbnb, VTTs) can read the modes off the effect stack; a separate engine slice would extend `getEffectiveSpeed` to consume `ModifySpeed` entries from the effect accumulator (would also retrofit Fast Movement etc.). |
