...
| Spells | 399 / 399 | ~399 | 34 / 60 / 63 / 54 / 40 / 46 / 38 / 24 / 19 / 21 across L0–L9. **Every PHB 2024 spell now ships in the pack.** ~136 wired with `mechanicalEffects` (~18 cantrips, ~29 L1, ~23 L2, ~17 L3, ~12 L4, ~13 L5, ~10 L6, ~5 L7, ~6 L8, ~3 L9) + 7 dedicated planners (counterspell, dispel-magic, identify, misty-step, shield, hunters-mark, polymorph). The remaining ~264 ship schema-only, each blocked on a named engine primitive captured in the per-level sections below. |
...
| **~~Caster-chosen options at cast time~~** ✓ shipped (partial) | 5 wired (chromatic-orb: damage type on attack; enlarge-reduce: variant on buff; calm-emotions: variant on save; command: 5-variant save; enhance-ability: 6-variant buff). ~3 still deferred: bestow-curse (one of four curse variants: pure data once cursed-X-active conditions are authored); elemental-weapon (damage type for weapon enhance + needs the item-buff damage-type extension); spirit-shroud (damage type for on-hit rider + needs the on-hit primitive extension). | Slice 82 introduced the `CasterChoice` discriminated-union on `CastSpellIntent` plus `casterChoosesDamageType: { allowed: DamageType[] }` on attack mechanics. Slice 83 added the `'variant'` discriminator case (`{ kind: 'variant', value: string }`) plus `casterChoosesVariant: { variants: [{ key, conditionId }] }` on buff mechanics. Slice 84 ported the same `casterChoosesVariant` shape to save mechanics — the variant key picks the condition that applies on a failed save. The buff and save resolvers share a `resolveVariantConditionId` helper. Slices 85 (Command, 5-variant save) and 86 (Enhance Ability, 6-variant buff) ship as pure data on top of the primitives. Same planner validation pattern: throws on missing / wrong-kind / unknown-key choices. Distinct from the `PendingChoice` protocol, which models deferred decisions persisted in state (level-up choices). |
