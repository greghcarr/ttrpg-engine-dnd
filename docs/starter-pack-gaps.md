...
| Spells | 399 / 399 | ~399 | 34 / 60 / 63 / 54 / 40 / 46 / 38 / 24 / 19 / 21 across L0–L9. **Every PHB 2024 spell now ships in the pack.** ~133 wired with `mechanicalEffects` (~18 cantrips, ~28 L1, ~21 L2, ~17 L3, ~12 L4, ~13 L5, ~10 L6, ~5 L7, ~6 L8, ~3 L9) + 7 dedicated planners (counterspell, dispel-magic, identify, misty-step, shield, hunters-mark, polymorph). The remaining ~267 ship schema-only, each blocked on a named engine primitive captured in the per-level sections below. |
...
**Wired (21):** aid, blindness-deafness, branding-smite (on-hit rider, one-shot), crown-of-madness, enlarge-reduce (caster-chosen variant), find-steed (summon), flame-blade, heat-metal, hold-person, invisibility, lesser-restoration, melfs-acid-arrow, moonbeam, prayer-of-healing, protection-from-poison, scorching-ray, shatter, spiritual-weapon, suggestion, summon-beast (summon), web.

**Dedicated planner (1):** misty-step.

**Schema-only (42):** grouped by the engine primitive each one needs.
...
- **Caster-chosen options at cast time** (variant chosen by caster, ability target chosen): `calm-emotions`, `enhance-ability`. (`enlarge-reduce` wired in slice 83 via the buff mechanic's `casterChoosesVariant` field; the on-hit ±1d4 weapon-damage rider stays deferred — needs a per-attack damage-die rider that isn't yet modeled.)
...
| **~~Caster-chosen options at cast time~~** ✓ shipped (partial) | 2 wired (chromatic-orb: caster-chosen damage type on attack mechanics; enlarge-reduce: caster-chosen variant on buff mechanics). ~6 still deferred: command (per-word effects), calm-emotions (suppress vs make indifferent), enhance-ability (Bear/Bull/Cat/Eagle/Fox/Owl), bestow-curse (one of four curse variants), elemental-weapon (damage type for weapon enhance + needs the item-buff damage-type extension), spirit-shroud (damage type for on-hit rider + needs the on-hit primitive extension). | Slice 82 introduced the `CasterChoice` discriminated-union on `CastSpellIntent` plus `casterChoosesDamageType: { allowed: DamageType[] }` on attack mechanics. Slice 83 added the `'variant'` discriminator case (`{ kind: 'variant', value: string }`) plus `casterChoosesVariant: { variants: [{ key, conditionId }] }` on buff mechanics. Same planner validation pattern: throws on missing / wrong-kind / unknown-key choices. The next variants to land are `'ability'` (enhance-ability picks STR / DEX / etc; needs an ability-check-buff condition with the chosen ability folded in) and `'condition'` (bestow-curse's pool of one-of-four curses; same `'variant'` shape but with several similarly-structured target conditions). Distinct from the `PendingChoice` protocol, which models deferred decisions persisted in state (level-up choices). |
