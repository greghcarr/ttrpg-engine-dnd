...
- `aura-of-protection` (L6) — friendlies within 10ft gain CHA-mod to saving throws. Needs a `sourceAbilityMod` Formula extension so the ally's save derivation can read the source paladin's CHA mod via the AppliedCondition's `sourceCharacterId` link. Deferred as a Formula-extension slice; not part of the slice 63 aura primitive.
...
| **~~Aura primitive~~** ✓ shipped (partial) | 2 wired (paladin Aura of Courage L10, Crusader's Mantle L3 spell); 1 still deferred (Aura of Protection L6 — needs the `sourceAbilityMod` Formula extension below) | Slice 63 adds the `GrantAura` effect (auraId, rangeFeet, allyConditionId) as discoverable metadata: consumers (dndbnb / DMs / VTTs) read the bearer's `GrantAura` effects and apply / remove the `allyConditionId` on in-range allies via `ConditionApplied` / `ConditionRemoved`. The engine doesn't auto-project (position is consumer territory). Aura conditions that don't need source-relative formulas (Aura of Courage's `GrantConditionImmunity`, Crusader's Mantle's weapon-hit rider) wire cleanly today. The Paladin's own native effect (Frightened immunity at L10) ships as a sibling effect on the same feature. Spell auras (Crusader's Mantle) use the existing `buff` mechanic with multi-target lists, plus an `OnEvent` rider condition in the same shape as Divine Favor. |
| **Source-relative Formula extension** | ~2 (aura-of-protection, future spells like Bless variants) | Add a `sourceAbilityMod` Formula kind that, given the condition's `AppliedCondition.sourceCharacterId`, evaluates the source character's ability mod. Lets aura conditions express "+CHA-mod-of-source to my save" without baking values at apply time. Bigger than the aura primitive (touches the formula evaluator + derive layer) but unblocks Aura of Protection and similar source-relative buffs across the catalog. |
