...
| **~~Area-effect spell mechanic~~** ✓ shipped (partial) | 10 wired (stinking-cloud, wall-of-fire, wall-of-thorns, wall-of-ice, blade-barrier, entangle, grease, black-tentacles, flaming-sphere, cloud-of-daggers) | The `aura-damage` mechanic has been extended over three slices to cover most of the persistent-area-effect family. Slice 68 made `damageDice` / `damageType` optional and added `conditionOnFail` (gated by `isImmuneToCondition`). Slice 69 wired 4 more spells onto the same primitive. Slice 70 made `saveAbility` optional too, so auto-damage zones like Cloud of Daggers (no save, just damage) wire cleanly. `planTickAura` now handles damage-only-with-save, condition-only-with-save, damage-only-no-save, and combined paths. Spells fire via `engine.plan.tickAura` — the consumer tracks "creature entered the zone" / "creature started its turn in it" and calls tickAura with that target. Remaining deferred area-effect spells (spike-growth, hunger-of-hadar, plant-growth, slow, dust-devil, silence, zone-of-truth, darkness, wind-wall, leomunds-tiny-hut) need additional primitives: damage-per-foot-moved (Spike Growth), recurring-rider for multi-component zones (Hunger of Hadar's cold-on-enter + acid-on-tick), pure terrain marker (Plant Growth), area-of-pure-obscurement / silence zones, and the on-cast targeting-and-save-then-buff pattern (Slow). |
