...
| Spells | 399 / 399 | ~399 | 34 / 60 / 63 / 54 / 40 / 46 / 38 / 24 / 19 / 21 across L0–L9. **Every PHB 2024 spell now ships in the pack.** ~137 wired with `mechanicalEffects` (~18 cantrips, ~29 L1, ~23 L2, ~18 L3, ~12 L4, ~13 L5, ~10 L6, ~5 L7, ~6 L8, ~3 L9) + 7 dedicated planners (counterspell, dispel-magic, identify, misty-step, shield, hunters-mark, polymorph). The remaining ~263 ship schema-only, each blocked on a named engine primitive captured in the per-level sections below. |
...
**Wired (29):** bane, bless, burning-hands, cause-fear, charm-person, chromatic-orb (caster-chosen damage type), color-spray, command (caster-chosen variant on save mechanic, 5 variants), cure-wounds, divine-favor (on-hit rider), dissonant-whispers, earth-tremor, faerie-fire, feather-fall (falling-protection buff), find-familiar (summon), guiding-bolt, healing-word, hellish-rebuke, inflict-wounds, mage-armor, magic-missile, ray-of-sickness, searing-smite (on-hit rider, one-shot), sleep, tashas-hideous-laughter, thunderous-smite (on-hit rider, one-shot), thunderwave, unseen-servant (summon), wrathful-smite (on-hit rider, one-shot).
...
**Wired (18):** animate-dead (summon), bestow-curse (caster-chosen 4-variant save; conditions are narrative-only pending the on-hit-add-damage + recurring-save-with-action-consume + directional-attack-disadvantage primitives), call-lightning, conjure-animals (summon), crusaders-mantle (aura buff), fear, fireball, hypnotic-pattern, lightning-bolt, mass-healing-word, phantom-steed (summon), sleet-storm, spirit-guardians, summon-fey (summon), summon-lesser-demons (summon), summon-shadowspawn (summon), summon-undead (summon), vampiric-touch.
