...
| **~~Area-effect spell mechanic~~** ✓ shipped (partial) | 5 wired (stinking-cloud, wall-of-fire, wall-of-thorns, wall-of-ice, blade-barrier) | Slice 68 extends the `aura-damage` mechanic: `damageDice` and `damageType` become optional (so condition-only zones like Stinking Cloud work), and a new `conditionOnFail` field applies a condition to creatures that fail the save (gated by `isImmuneToCondition`, so Aura of Courage etc. block Frightened-style projections cleanly). `planTickAura` handles both paths. Spells fire via `engine.plan.tickAura` per-turn — the consumer is responsible for tracking "creature entered the zone" or "creature started its turn in the zone" and calling tickAura with that target. Remaining area-effect spells (entangle, grease, cloud-of-daggers, spike-growth, hunger-of-hadar, plant-growth, slow, dust-devil, flaming-sphere, silence, zone-of-truth, darkness, wind-wall, leomunds-tiny-hut) still need additional primitives: difficult-terrain marker, recurring-rider for damage-per-turn-in-zone, auto-damage without save, two-damage-source zones, etc. |
