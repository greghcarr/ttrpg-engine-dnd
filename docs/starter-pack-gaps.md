...
| Spells | 399 / 399 | ~399 | 34 / 60 / 63 / 54 / 40 / 46 / 38 / 24 / 19 / 21 across L0–L9. **Every PHB 2024 spell now ships in the pack.** ~132 wired with `mechanicalEffects` (~18 cantrips, ~28 L1, ~20 L2, ~17 L3, ~12 L4, ~13 L5, ~10 L6, ~5 L7, ~6 L8, ~3 L9) + 7 dedicated planners (counterspell, dispel-magic, identify, misty-step, shield, hunters-mark, polymorph). The remaining ~268 ship schema-only, each blocked on a named engine primitive captured in the per-level sections below. |
...
| **~~Caster-chosen options at cast time~~** ✓ shipped (partial) | 1 wired (chromatic-orb: caster-chosen damage type on attack mechanics). ~7 still deferred: command (per-word effects), calm-emotions (suppress vs make indifferent), enhance-ability (Bear/Bull/Cat/Eagle/Fox/Owl), enlarge-reduce (enlarge vs reduce), bestow-curse (one of four curse variants), elemental-weapon (damage type for weapon enhance + needs the item-buff damage-type extension), spirit-shroud (damage type for on-hit rider + needs the on-hit primitive extension). | Slice 82 adds the `CasterChoice` type (`{ kind: 'damageType', value: DamageType }` for now) on `CastSpellIntent`, plus an optional `casterChoosesDamageType: { allowed: DamageType[] }` field on attack mechanics. The planner validates at cast time (throws on missing / mismatch / out-of-list) and uses the chosen type for both `DamageRolled` and the mitigation step. Distinct from the `PendingChoice` protocol, which models deferred decisions persisted in state (level-up choices). Cast-time choices are inline and immediate; the next variants to model are `'variant'` (e.g. enlarge vs reduce) and `'condition'` (e.g. bestow-curse's curse picks) — each adds a discriminator case to `CasterChoice` plus a parallel field on the relevant mechanic kind. |
