...
| **~~Area-effect spell mechanic~~** ✓ shipped (partial) | 11 wired (stinking-cloud, wall-of-fire, wall-of-thorns, wall-of-ice, blade-barrier, entangle, grease, black-tentacles, flaming-sphere, cloud-of-daggers, hunger-of-hadar) | The `aura-damage` mechanic has been extended over four slices to cover most of the persistent-area-effect family. Slice 68 made `damageDice` / `damageType` optional and added `conditionOnFail` (gated by `isImmuneToCondition`). Slice 69 wired 4 more spells onto the same primitive. Slice 70 made `saveAbility` optional so auto-damage zones like Cloud of Daggers wire cleanly. Slice 71 adds an optional `trigger: 'on-enter' \| 'on-turn-start' \| 'on-turn-end'` field plus a matching `intent.trigger` on `tickAura` — multi-component zones like Hunger of Hadar (2d6 cold on enter, 2d6 acid with DEX save on turn end) ship both components as sibling mechanics and the consumer picks which fires per call. Mechanics without a trigger remain "fires on every call" for backward compat. `planTickAura` filters mechanics by trigger and iterates each. Remaining deferred area-effect spells (spike-growth, plant-growth, slow, dust-devil, silence, zone-of-truth, darkness, wind-wall, leomunds-tiny-hut) need additional primitives: damage-per-foot-moved (Spike Growth), pure terrain marker (Plant Growth), area-of-pure-obscurement / silence zones, and the on-cast targeting-and-save-then-buff pattern (Slow). |
