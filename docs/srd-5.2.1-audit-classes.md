...
- **Cleric**: ~~Divine Order (L1)~~ (slice 214: 2-option OfferChoice; Protector grants martial weapon + heavy armor proficiency, Thaumaturge grants Guidance cantrip + WIS-mod bonus on Arcana/Religion checks. RAW lets the cantrip be any cleric cantrip; the pack hardcodes Guidance to avoid a nested OfferChoice for now), **Greater Divine Intervention (L20)** — base Divine Intervention now ships at L10 via slice 220 (`planDivineIntervention` + new `ignorePreparation` CastSpellIntent flag; consumes the `divine-intervention` resource, delegates to planCastSpell with noSlotCost + ignorePreparation; validates Cleric-list + L5-or-lower + non-Reaction). The L20 Wish extension still defers pending a marker primitive that widens the selectable set plus the 2d4-LR cooldown override on Wish.
