...
- **Cleric**: ~~Divine Order (L1)~~ (slice 214: 2-option OfferChoice; Protector grants martial weapon + heavy armor proficiency, Thaumaturge grants Guidance cantrip + WIS-mod bonus on Arcana/Religion checks. RAW lets the cantrip be any cleric cantrip; the pack hardcodes Guidance to avoid a nested OfferChoice for now), **Greater Divine Intervention (L20)** — deferred until base Divine Intervention (Cleric L10) ships as a dedicated planner. The L20 feature only extends Divine Intervention's spell-selection list to include Wish, so there's nothing to wire on top of an empty base. Multi-slice prerequisite: ship planDivineIntervention (resource consume, optional spell selection from cleric list, 2d4-LR cooldown on success per RAW), then the L20 grant becomes a marker primitive that adds Wish to the selectable list.
...
- **Ranger**: ~~Expertise (L9)~~ (slice 208: pure content; OfferChoice over the 8 ranger skills, mirrors Rogue Expertise), **Precise Hunter (L17)** — deferred until Hunter's Mark (Ranger L1 spell) ships as a dedicated planner. The spell currently has `mechanicalEffects: []`; no `hunters-mark-active` condition exists on targets with a source link. Precise Hunter's RAW ("while your Hunter's Mark spell is on a creature, you have Advantage on attack rolls against that creature") needs a 3-way join: the bearer carries a marker, the target carries the `hunters-mark-active` condition, and that condition's `sourceCharacterId` matches the attacker. Multi-slice prerequisite (planHuntersMark first, then a `GrantAdvantageVsBearersOfMyCondition { conditionId }` primitive on the L17 feature).
...
About 17 main-class features at slice 196; slices 199-216 closed 14 of them: Rogue L5 + L18 (slices 200, 199), Sorcerer L7 alt-cost arm (slice 201), Monk L6 + L10 + L14 + L18 (slices 207, 202, 203, 209), Ranger L9 (slice 208), Paladin L2 + L18 (slices 210, 211), Cleric L1 (slice 214), Druid L1 (slice 215), Bard L20 + Warlock L9 (slice 216). **Three remain deferred with multi-slice prerequisites** (annotated inline below): Cleric L20 Greater Divine Intervention (needs base Divine Intervention planner first), Monk L10 Heightened Focus (needs Flurry of Blows / Patient Defense / Step of the Wind planners first), Ranger L17 Precise Hunter (needs Hunter's Mark planner + a new `GrantAdvantageVsBearersOfMyCondition` primitive). The doubled-metamagic arm of Sorcery Incarnate is also still deferred (needs once-per-spell metamagic enforcement).
