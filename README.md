...
- **Content**: 399 spells (every PHB 2024 entry, ~162 mechanically wired); 12 classes with 1–20 level tables fully populated (slices through L20 across all 12); 12 subclasses (one canonical per class, L3 only); 16 of 16 PHB 2024 backgrounds (complete); 35 conditions (all 15 RAW plus 20 mechanic-rider variants); ~33 feats; 9 magic items; 6 monster statblocks.
...
| ~~Auto-expiry for trigger-applied conditions~~ | 🟢 | `ApplyCondition` TriggerAction (slice 98) stamps `durationRounds` on the action, the dispatcher converts it to `expiresOnRound` on the emitted ConditionApplied when inside an active encounter (slice 102), and `planAdvanceTurn` sweeps the appliedConditions on each turn-start emitting `ConditionRemoved` for any whose source matches the new active combatant and whose expiry has arrived. | Spirit Shroud's heal-block now auto-expires at the start of the caster's next turn per RAW. Future trigger-applied conditions with `durationRounds` pick up the same auto-expiry for free. Outside an encounter the expiry stays consumer-managed (no round counter to anchor against). |
...
| Spells | 399 (~162 fully wired) | 399 (PHB 2024 complete) | 🟢 | Every PHB 2024 spell ships in the pack across L0–L9 (34 / 60 / 63 / 54 / 40 / 46 / 38 / 24 / 19 / 21). ~153 wired via `mechanicalEffects` plus 9 dedicated planners (absorb-elements, counterspell, dispel-magic, elemental-weapon, identify, misty-step, shield, hunters-mark, polymorph). The remaining ~237 ship schema-only and each names the specific engine primitive blocking it. See [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md) for the per-spell catalog and which primitive each deferred spell waits on. |
...
| Conditions | 35 (15 RAW + 20 rider) | 15 RAW | ✓ | All 15 RAW conditions plus 20 mechanic-rider conditions the engine uses (`blessed`, `baned`, `held-paralyzed-active`, `cursed-attacks-active`, `fire-shield-warm-active`, `healing-blocked-active`, `protection-from-evil-and-good-active`, `holy-aura-active`, `blade-warded-active`, `darkvision-active`, etc.). |
