...
**Alpha.** Architecturally complete and content-substantial. 1471 tests across 203 files; the engine compiles and builds (ESM + CJS + `.d.ts`); the load-bearing invariants (event-sourcing, plan/commit, RNG capture, replay equivalence, branded IDs, effect primitives) are locked and proven. The 48-probe RAW-compliance audit at [tests/audit/raw-compliance.test.ts](tests/audit/raw-compliance.test.ts) passes in full.
...
|---|---|---|
| **Full (achievable) SRD-compliance** | `█████████████░░░░░░░` 🟨 ~65% | Weighted across the rows below: engine + primitives heavy weight (currently high), content rows lower. SRD 5.2.1's narrower content scope (vs full PHB / DMG / MM) is what makes the number reachable; the long content tail (DMG items, MM bestiary) is the floor on how fast this moves. Slices 141 to 149 standardized the entire monster cohort against SRD 5.2.1 (111/111 exact-match on AC + HP + abilities + CR; audit at [docs/srd-5.2.1-audit.md](docs/srd-5.2.1-audit.md)). |
| Engine architecture | `████████████████████` 🟩 100% | event-sourcing, plan/commit, RNG capture, replay equivalence, branded IDs, dispatcher: all locked |
| Effect-primitive vocabulary | `█████████████░░░░░░░` 🟨 ~65% | ~31 primitives wired (+ `PreventFatalDamage` from slice 111); per-modifier predicate gates now honored (slices 115/116/117). 15–25 still on the menu in [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md) |
...
| Spells (mechanically wired) | `████████░░░░░░░░░░░░` 🟧 42% | ~158/399; remainder names the engine primitive each blocks on |
| Conditions | `████████████████████` 🟩 100% | 15/15 RAW + 24 mechanic-rider variants the engine emits |
| Magic items | `███████████████░░░░░` 🟨 ~69% | 138 items spanning common through legendary; 31 charges-bearing |
| Monsters | `██████░░░░░░░░░░░░░░` 🟥 ~30% | 111/~370 MM statblocks across all 14 creature types (CR 0 to 21) |
...
| Spells | 399 (~165 fully wired) | 399 (PHB 2024 complete) | 🟢 | Every PHB 2024 spell ships in the pack across L0–L9 (34 / 60 / 63 / 54 / 40 / 46 / 38 / 24 / 19 / 21). ~155 wired via `mechanicalEffects` plus 10 dedicated planners (absorb-elements, counterspell, dispel-magic, elemental-weapon, identify, misty-step, shield, sanctuary, hunters-mark, polymorph). The remaining ~234 ship schema-only and each names the specific engine primitive blocking it. See [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md) for the per-spell catalog and which primitive each deferred spell waits on. |
| Magic items | 138 | hundreds (DMG) | 🟢 | Spans common through legendary tiers. Includes the alpha.5 seed, the wondrous fillout (items batches 1.1 through 1.8, 1.10, 1.12 through 1.18), 8 named magic weapons (Vicious / Sun Blade / Frost Brand / Holy Avenger / Vorpal Sword / Dwarven Thrower / Hammer of Thunderbolts / Berserker Axe), 8 named armors and shields, +1 / +2 / +3 weapon and armor templates, the full Giant Strength belt and potion family, 8 Ioun Stones, 16 rings, 8 robes / cloaks / outer garments, 8 cloaks and small-slot accessories, and 30 charges-bearing items (rods / staves / wands / charged rings + the alpha.5 Wand of Magic Missiles). 30 consumables (16 potions + 8 spell scrolls cantrip-through-9th + 6 alpha.5). Wiring of most items' effects arrays remains deferred behind the magic-item-effect-projection pass (slice 132 unblocked it; content-side wiring is the remaining work). |
| Monster statblocks | 111 | ~370 (MM) | 🟢 | All 14 MM creature types now in pack: Humanoid, Beast, Undead, Giant, Dragon, Fiend, Elemental, Monstrosity, Ooze, Construct, Aberration, Fey, Celestial, Plant. CR 0 (Awakened Shrub, Lemure, Commoner) to CR 21 (Solar). 12 statblocks with wired Recharge area-save Breath Weapons (all dragons + Iron Golem + Ankheg, since slice 140), 6 with Legendary Actions and Resistance (Aboleth + 5 Adult Chromatics), 16 with Magic Resistance (wired since slice 131). Highest-leverage remaining engine primitives are Legendary Actions / Resistance, Frightful Presence, Innate Spellcasting per-spell envelope, Charge / Pounce / Trampling, and an unconditional elemental rider on bite. Slices 141 to 149 standardized the cohort against SRD 5.2.1: 7 non-SRD entries dropped (slice 141), 9 entries renamed to SRD 5.2.1 names with stat refresh (slice 142), 56 entries refreshed for stat drift (slices 143 to 148). All 111 entries now match SRD 5.2.1 exactly on AC, HP, abilities, and CR. Audit at [docs/srd-5.2.1-audit.md](docs/srd-5.2.1-audit.md). |
