...
|---|---|---|
| **Full (achievable) SRD-compliance** | `███████████████░░░░░` 🟨 ~75% | Weighted across the rows below: engine + primitives heavy weight (currently high), content rows lower. SRD 5.2.1's narrower content scope (vs full PHB / DMG / MM) is what makes the number reachable; the long content tail (DMG items, MM bestiary) is the floor on how fast this moves. Slices 141-149 standardized monsters (initial 111 entries); slices 150-153 audited items / spells / classes / character-creation; slices 177-196 swept ~310 drift fixes across every script-detectable field (schools, levels, components, ranges, durations, concentration, ritual flags, class lists, damage dice, V/S/M, attunement, weapon mastery, feat prereqs, tool categorization). Slice 195 codified the audit logic as a checked-in vitest harness. Monster lane batches 4.1-4.14 added 63 monsters (118 to 181); item lane batches 4.1-4.16 added 113 SRD adventuring-gear / tools / consumables (217 to 330 items). |
| Engine architecture | `████████████████████` 🟩 100% | event-sourcing, plan/commit, RNG capture, replay equivalence, branded IDs, dispatcher: all locked |
| Effect-primitive vocabulary | `█████████████░░░░░░░` 🟨 ~65% | 42 primitives wired (current `EFFECT_KINDS` in [src/schemas/effects.ts](src/schemas/effects.ts)); per-modifier predicate gates now honored (slices 115/116/117). Slice 199 added `CancelAdvantageOnAttackers` for Rogue L18 Elusive; slice 200 added `GrantUncannyDodge` + dedicated `planUncannyDodge` reaction planner for Rogue L5 Uncanny Dodge; slice 201 added `GrantInnateSorcerySpendAlternative` + dedicated `planInnateSorcery` for Sorcerer L7 Sorcery Incarnate (alternative-cost arm). 15–25 still on the menu in [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md) |
...
| Subclasses | `█████░░░░░░░░░░░░░░░` 🟧 24% | 12/12 SRD 5.2.1 subclasses (one per class); each ships L3 features but L6 / L10 / L14 / L17 are mostly missing (41 SRD-listed subclass features absent from pack; full list in [docs/srd-5.2.1-audit-classes.md](docs/srd-5.2.1-audit-classes.md)) |
...
| Conditions | `████████████████████` 🟩 100% | 15/15 RAW + 24 mechanic-rider variants the engine emits |
...
| Monsters | `██████████░░░░░░░░░░` 🟨 ~49% | 181/~370 MM statblocks across all 14 creature types (CR 0 to 24). Both Chromatic + Metallic Dragon ladders fully closed (40 statblocks). |
...
| Monster statblocks | 181 | ~370 (MM) | 🟢 | All 14 MM creature types now in pack: Humanoid, Beast, Undead, Giant, Dragon, Fiend, Elemental, Monstrosity, Ooze, Construct, Aberration, Fey, Celestial, Plant. CR 0 (Awakened Shrub, Lemure, Commoner) to CR 24 (Ancient Red + Ancient Gold). Both Chromatic + Metallic Dragon ladders fully closed (40 statblocks). 42 statblocks with wired Recharge area-save Breath Weapons (every dragon + Iron Golem + Ankheg, since slice 140), 6 with Legendary Actions and Resistance (Aboleth + 5 Adult Chromatics), 30 with Magic Resistance (wired since slice 131). Highest-leverage remaining engine primitives are Legendary Actions / Resistance, Frightful Presence, Innate Spellcasting per-spell envelope, Charge / Pounce / Trampling, and an unconditional elemental rider on bite. Slices 141-149 standardized the initial 111 entries against SRD 5.2.1. Monster lane batches 4.1-4.14 added 63 SRD-aligned statblocks (mid-tier Demon + Devil ladders, complete Chromatic + Metallic Dragon trees, Vampire/Mummy roster, lycanthropes, Hag trio, Genie + Mephit cohort, Monstrosities). All 181 entries verified against the slice-195 SRD drift harness. Audit at [docs/srd-5.2.1-audit.md](docs/srd-5.2.1-audit.md). |
...
| Conditions | 37 (15 RAW + 22 rider) | 15 RAW | ✓ | All 15 RAW conditions plus 22 mechanic-rider conditions the engine uses (`blessed`, `baned`, `held-paralyzed-active`, `cursed-attacks-active`, `fire-shield-warm-active`, `healing-blocked-active`, `protection-from-evil-and-good-active`, `holy-aura-active`, `blade-warded-active`, `darkvision-active`, `sanctuary-active`, `studied-target-active`, etc.). |
