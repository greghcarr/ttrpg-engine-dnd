...
| Fighting Style gates | 🟢 | Archery's +2 attack gated on ranged attacks (slice 115). Defense's +1 AC gated on wearing armor (slice 116). Dueling's +2 damage gated on melee-attack + off-hand-not-weapon (slice 117). Two-Weapon Fighting adds the ability mod to off-hand damage via a new `GrantTwoWeaponFighting` marker effect (slice 119). All four Fighting Style gates from the 2024 PHB now apply RAW-correctly. Slice 117 also surfaced + fixed a latent miss: the `target: 'damage'` modifier sum was never being consumed by any planner; Frenzy's +2 damage was also dormant and is now wired. | Great Weapon Fighting (reroll 1s/2s on damage dice) and Protection (reactive impose-disadvantage on attacks against allies) ship as `effects: []`. Each is its own mechanic; the 2024 PHB has 6 Fighting Styles total — 4 are wired (Archery, Defense, Dueling, Two-Weapon), 2 remain. |
...
### Post-alpha.5: vocabulary expansion (slices 48–119, ongoing)
...
- **Engine primitives** (~35 slices). New Effect kinds, TriggerAction variants, and planners. Highlights:
  ...
  - Predicate-gated `AddModifier` (slice 115) + `bearer.wearingArmor` fact (slice 116) + Dueling damage path (slice 117) + Two-Weapon Fighting marker (slice 119): all four 2024-PHB Fighting Styles that fit the predicate / planner-flag pattern (Archery, Defense, Dueling, Two-Weapon) now apply RAW-correctly. Slice 117 also activated the dormant `target: 'damage'` modifier sum (Frenzy's +2 damage lights up incidentally). The remaining two Fighting Styles (Great Weapon Fighting, Protection) each need their own mechanic.
