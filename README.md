...
| Effect-primitive vocabulary | `█████████████░░░░░░░` 🟨 ~70% | 46 primitives wired (current `EFFECT_KINDS` in [src/schemas/effects.ts](src/schemas/effects.ts)); per-modifier predicate gates now honored (slices 115/116/117). Slices 199-207 added six marker primitives (CancelAdvantageOnAttackers, GrantUncannyDodge, GrantInnateSorcerySpendAlternative, GrantSelfRestoration, GrantMaxHealingDice, GrantUnarmedAsMagical) plus four dedicated planners (Uncanny Dodge, Innate Sorcery, Self-Restoration, save-prof bug fix); slices 208-210 added Ranger Expertise content, Superior Defense planner, and Paladin's Smite planner; **slice 211 added `ExpandAuraRange` for Paladin L18 Aura Expansion (replaces the prior dedup-by-id workaround)**. 15–25 still on the menu in [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md) |
