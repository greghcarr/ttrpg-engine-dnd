...
|---|---|---|
| **Full (achievable) SRD-compliance** | `██████████████░░░░░░` 🟨 ~70% | Weighted across the rows below: engine + primitives heavy weight (currently high), content rows lower. SRD 5.2.1's narrower content scope (vs full PHB / DMG / MM) is what makes the number reachable; the long content tail (DMG items, MM bestiary) is the floor on how fast this moves. Slices 141 to 149 standardized monsters (111/111 exact-match on AC + HP + abilities + CR), slice 150 audited and pared magic items (122/122 SRD-derived), slice 151 audited spells (324/336 SRD-derived; 12 wired-non-SRD extras kept by policy). |
