...
| Fighting Style gates | ⚪ | Archery's +2 attack is now gated on ranged attacks (slice 115); Defense's +1 AC is gated on wearing armor (slice 116). Both wired across the standalone feat plus the Fighter / Paladin / Ranger Fighting Style choice options. | Dueling still applies +2 damage unconditionally (needs `bearer.offHandKind` fact + damage-modifier-path threading). Great Weapon Fighting (reroll 1s/2s on damage dice), Protection (reactive impose-disadvantage on attacks against allies), Two-Weapon Fighting (add ability mod to off-hand damage) ship as `effects: []`. Each is its own mechanic. |
...
### Post-alpha.5: vocabulary expansion (slices 48–116, ongoing)
...
- **Engine primitives** (~35 slices). New Effect kinds, TriggerAction variants, and planners. Highlights:
  ...
  - Predicate-gated `AddModifier` (slice 115) + `bearer.wearingArmor` fact (slice 116): Archery's +2 attack is gated on ranged attacks, Defense's +1 AC is gated on wearing armor. Same plumbing accommodates future `bearer.*` / `event.*` facts.
