...
- **Partially wired** (machinery exists, scope is narrower than the slice title implies):
  ...
  - Spells: ~212 spells in the pack (34 cantrips + 60 L1 + 63 L2 + 54 L3 + 1 L4); ~78 of those have full mechanical effects wired (~71 via `mechanicalEffects`, 7 via dedicated planners). Cantrips, L1, L2, and L3 each ship every PHB 2024 entry; deferred spells are schema-only and blocked on specific engine primitives (on-hit trigger system, reaction system, area-effect mechanic, temp-HP grant, summon system, AC-buff condition, composite-buff condition, etc). See [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md) for the per-spell catalog. L4 + L5-9 still mostly placeholder.
...
| Spells | 212 (~78 fully wired) | ~370 | 🟠 | Cantrips, L1, L2, and L3 each ship every PHB 2024 entry (34 / 60 / 63 / 54). L1: ~20 wired via `mechanicalEffects` plus 4 dedicated planners; ~36 schema-only. L2: ~17 wired plus 1 planner; ~45 schema-only. L3: ~9 wired plus 2 planners (Counterspell, Dispel Magic); ~43 schema-only. L4 has Polymorph (planner). L4 remainder + L5-9 is still consumer territory. See [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md) for the per-spell catalog and which primitive each deferred spell waits on. |
