...
- **Content**: 399 spells (every PHB 2024 entry, ~155 mechanically wired); 12 classes with 1–20 level tables fully populated (slices through L20 across all 12); 12 subclasses (one canonical per class, L3 only); 16 of 16 PHB 2024 backgrounds (complete); 28 conditions (all 15 RAW plus 13 mechanic-rider variants); ~33 feats; 9 magic items; 6 monster statblocks.
...
| Spells | 399 (~155 fully wired) | 399 (PHB 2024 complete) | 🟢 | Every PHB 2024 spell ships in the pack across L0–L9 (34 / 60 / 63 / 54 / 40 / 46 / 38 / 24 / 19 / 21). ~146 wired via `mechanicalEffects` plus 9 dedicated planners (absorb-elements, counterspell, dispel-magic, elemental-weapon, identify, misty-step, shield, hunters-mark, polymorph). The remaining ~244 ship schema-only and each names the specific engine primitive blocking it. See [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md) for the per-spell catalog and which primitive each deferred spell waits on. |
