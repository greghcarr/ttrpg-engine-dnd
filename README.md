...
- **Partially wired** (machinery exists, scope is narrower than the slice title implies):
  ...
  - Spells: 251 spells in the pack (34 cantrips + 60 L1 + 63 L2 + 54 L3 + 40 L4); ~100 of those have full mechanical effects wired (93 via `mechanicalEffects`, 7 via dedicated planners). Cantrips, L1, L2, L3, and L4 each ship every PHB 2024 entry; deferred spells are schema-only and blocked on specific engine primitives (area-effect spell mechanic, sensor / scrying, terrain shaping, action-table riser for Confusion, AddDamageToAttacker TriggerAction for Fire Shield, etc.). The on-hit trigger primitive (slice 61) wires the smite cohort + Cleric Blessed Strikes; the aura primitive (slice 63) + the `sourceAbilityMod` Formula extension (slice 64) wire Paladin's Aura of Protection / Courage and Crusader's Mantle. See [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md) for the per-spell catalog and a backlog of remaining engine primitives. L5–9 still consumer territory.
...
| Spells | 251 (~100 fully wired) | ~370 | 🟠 | Cantrips, L1, L2, L3, and L4 each ship every PHB 2024 entry (34 / 60 / 63 / 54 / 40). L1: ~26 wired via `mechanicalEffects` plus 4 dedicated planners. L2: ~20 wired plus 1 planner. L3: ~17 wired plus 2 planners (Counterspell, Dispel Magic). L4: ~12 wired plus 1 planner (Polymorph); 27 schema-only awaiting area-effect, sensor / scrying, terrain shaping, action-table, and other primitives. The summon system (slice 48) wires the 11 + 4 summon-pattern spells. On-hit trigger (slice 61) wires the smite cohort. Aura primitive (slice 63) + sourceAbilityMod Formula (slice 64) wire the paladin auras + Crusader's Mantle. L5-9 is still consumer territory. See [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md) for the per-spell catalog and which primitive each deferred spell waits on. |
