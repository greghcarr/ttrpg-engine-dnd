...
- **Partially wired** (machinery exists, scope is narrower than the slice title implies):
  ...
  - Spells: ~105 spells in the pack (34 cantrips + 60 L1 + 6 L2 + 4 L3 + 1 L4); ~52 of those have full mechanical effects wired (~45 via `mechanicalEffects`, 7 via dedicated planners). The cantrip layer is feature-complete; the L1 layer ships every PHB 2024 L1 spell but ~36 are schema-only, blocked on engine primitives (on-hit trigger system, reaction system, area-effect mechanic, temp-HP grant, summon system, AC-buff condition). See [dndbnb/BACKLOG.md](dndbnb/BACKLOG.md#engine-side-asks-blocking-dndbnb-features) for the per-spell catalog. Utility cantrips and pure-narrative L1s (Comprehend Languages, Detect Evil and Good, etc.) are intentionally event-free.
...
| Spells | 105 (~52 fully wired) | ~370 | 🟠 | Cantrips: 34 of the ~34 PHB 2024 cantrips ship. Leveled spells: 60 L1 / 6 L2 / 4 L3 / 1 L4 ship; L2 onward is still consumer territory. Of the 60 L1, ~20 are fully wired with mechanicalEffects (Bless, Burning Hands, Cause Fear, Charm Person, Color Spray, Cure Wounds, Dissonant Whispers, Earth Tremor, Faerie Fire, Guiding Bolt, Healing Word, Hellish Rebuke, Hold Person, Inflict Wounds, Mage Armor, Magic Missile, Ray of Sickness, Sleep, Tasha's Hideous Laughter, Thunderwave), plus 4 with dedicated planners (Shield, Hunter's Mark, Identify, Misty Step); the remaining ~36 L1 spells ship schema-only, blocked on engine primitives (on-hit trigger system, reaction system, area-effect mechanic, temp-HP grant, summon system, AC-buff condition). See [dndbnb/BACKLOG.md](dndbnb/BACKLOG.md#engine-side-asks-blocking-dndbnb-features) for the per-spell catalog and which primitive each needs. Utility cantrips remain intentionally narrative-only. |
