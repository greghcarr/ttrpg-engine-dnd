...
- ◐ **Slice 41.** Spell catalog: ~33 spells across cantrips + L1-3 + a single L4 (Polymorph) covering common archetypes. **Of those, ~21 have full mechanical effects wired and ~10 are schema-only TODOs**: Sleep (HP-threshold knockout, needs a new mechanic kind), Shield (reactive +5 AC, needs reaction-interrupt timing), Misty Step (movement, no roll), Guidance (single-use buff that expires on first ability check), Spirit Guardians (damaging aura with per-turn ticks), plus the utility cantrips (Mage Hand, Prestidigitation, Light, Detect Magic) which are intentionally narrative-only. Filling the full 2024 catalog (~370 spells) is consumer territory; the schema and planners support every required shape but the JSON content is sparse.
