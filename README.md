...
- **Partially wired** (machinery exists, scope is narrower than the slice title implies):
  ...
  - Spells: ~162 spells in the pack (34 cantrips + 60 L1 + 63 L2 + 4 L3 + 1 L4); ~69 of those have full mechanical effects wired (~62 via `mechanicalEffects`, 7 via dedicated planners). Cantrips, L1, and L2 each ship every PHB 2024 entry; deferred spells at those levels are schema-only and blocked on specific engine primitives (on-hit trigger system, reaction system, area-effect mechanic, temp-HP grant, summon system, AC-buff condition, etc). See [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md) for the per-spell catalog. L3+ is still mostly L3 / L4 placeholders.
...
| Spells | 162 (~69 fully wired) | ~370 | 🟠 | Cantrips, L1, and L2 each ship every PHB 2024 entry (34 / 60 / 63). At L1: ~20 wired via `mechanicalEffects` plus 4 via dedicated planners; ~36 schema-only blocked on specific engine primitives. At L2: ~17 wired plus 1 dedicated planner (Misty Step); ~45 schema-only. L3 has 4 spells (Fireball, Spirit Guardians, Counterspell, Dispel Magic); L4 has Polymorph. L3+ remainder + L5-9 is still consumer territory. See [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md) for the per-spell catalog and which primitive each deferred spell waits on. |
