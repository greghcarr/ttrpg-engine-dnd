...
| Fighting Style gates | ⚪ | Archery's +2 attack gated on ranged attacks (slice 115). Defense's +1 AC gated on wearing armor (slice 116). Dueling's +2 damage gated on melee-attack + off-hand-not-weapon (slice 117). Slice 117 also surfaced + fixed a latent miss: the `target: 'damage'` modifier sum was never being consumed by any planner; Frenzy's +2 damage was also dormant and is now wired (with a melee predicate). | Great Weapon Fighting (reroll 1s/2s on damage dice), Protection (reactive impose-disadvantage on attacks against allies), Two-Weapon Fighting (add ability mod to off-hand damage) ship as `effects: []`. Each is its own mechanic. |
...
### Post-alpha.5: vocabulary expansion (slices 48–117, ongoing)
...
- **Engine primitives** (~35 slices). New Effect kinds, TriggerAction variants, and planners. Highlights:
  ...
  - Predicate-gated `AddModifier` (slice 115) + `bearer.wearingArmor` fact (slice 116) + Dueling damage path (slice 117): Archery's +2 attack is gated on ranged attacks; Defense's +1 AC on wearing armor; Dueling's +2 damage on melee + off-hand-not-weapon. Slice 117 also activated the dormant `target: 'damage'` modifier sum (which had never been consumed) — Frenzy's +2 damage incidentally lights up at the same time.
