...
| Effect-primitive vocabulary | `██████████████░░░░░░` 🟨 ~75% | **49 wired primitives** (current `EFFECT_KINDS` in [src/schemas/effects.ts](src/schemas/effects.ts)) plus the `Custom` escape hatch. Recent additions: slice 229 `OverrideAbilityScore` (Amulet of Health + Gauntlets + 6 Giant Strength belts), slice 231 `GrantAdvantageVsBearersOfMyCondition` (Ranger L17 Precise Hunter), slice 232 `Regeneration` (Troll + Troll Limb), slice 233 `SpawnCreature` TriggerAction (Troll's Loathsome Limbs). Plus new predicate facts: `bearer.wieldingShield` (slice 230), `event.damageOfType.<type>` (slice 233). The remaining backlog is enumerated in [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md) "Deferred primitives backlog". |
...
| Magic items | `███████████████████░` 🟩 ~99% pack-presence, 🟧 ~10% mechanically wired | **276 magic items in pack**, every SRD 5.2.1 H4 entry covered at the canonical-variant level (slice 224 added 149 missing entries; slice 225 added 20 variant unrolls). **27 mechanically wired** (slice 227 added 4, slice 229 added 8 OverrideAbilityScore wires, slice 230 added 1). Wiring the remaining ~245 requires `ConsumeItem` / `UseItem` planners + smaller per-item predicates (full inventory in [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md)). |
...
| Magic items | 276 | ~264 (SRD 5.2.1 magic-item A-Z) | 🟢 | **Pack-presence complete at the canonical-variant level** (slice 224 added 149 missing SRD entries; slice 225 added 20 variant unrolls for the 4 multi-rarity parents). Spans common through legendary. **27 mechanically wired** (slice 227 added 4, slice 229 added 8 OverrideAbilityScore wires for Amulet of Health + Gauntlets of Ogre Power + 6 Giant Strength belt variants, slice 230 added Bracers of Defense). Wiring the remaining ~245 requires `ConsumeItem` / `UseItem` planners + smaller per-item predicates. |
