...
**Alpha.** Architecturally complete; mechanically partial. 563 tests across 90 files; the engine compiles and builds (ESM + CJS + `.d.ts`); the load-bearing invariants (event-sourcing, plan/commit, RNG capture, replay equivalence, branded IDs, effect primitives) are locked and proven.
...
|---|---|---|---|---|
| Classes (table scaffolding) | 12 of 12 | 12 | 🟡 | Tables ship 1-20. Class-feature wire status (per the coverage matrix): **36 wired / 12 stub** at L1-5. Wired highlights: Sneak Attack scaling all 10 odd levels, Second Wind / Action Surge / Extra Attack, Rage resource, Bardic Inspiration + Expertise (Insight/Persuasion) + Font of Inspiration, Channel Divinity, Wild Shape, Monk's Focus / Unarmored Defense / Extra Attack, Hunter's Mark + Weapon Mastery grant, Innate Sorcery / Font of Magic / Sorcerous Restoration, Lay on Hands, Arcane Recovery, Barbarian Unarmored Defense + Danger Sense. Stubs (engine work not yet done): Martial Arts die scaling, Stunning Strike, Reckless Attack timing, Metamagic, Evasion, Druidic, Slow Fall, Cutting Words, Jack of All Trades (needs conditional predicate), Fighting Style (needs choice), etc. — flagged individually in the feature-coverage matrix. |
| Subclasses | 12 (1 per class, L3 only) | ~50+ | 🟡 | One canonical PHB-2024 subclass per class lands at L3 — the gating level. The L3 features are a mix of wired (Draconic Resilience AC, Thief Second-Story climb speed, College of Lore proficiencies, Champion Remarkable Athlete) and content-stub (Improved Critical, Frenzy, Disciple of Life, Sacred Weapon — each blocked by engine work not yet done). L7 / L10 / L14 feature grants and the other 3-4 subclasses per class are still consumer territory. |
