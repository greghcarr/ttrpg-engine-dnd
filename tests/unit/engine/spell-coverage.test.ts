...
const SPELL_EXPECTATIONS: Record<string, Expectation> = {
  ...
  'black-tentacles': { kind: 'skip', reason: '20-ft cube area-effect with on-enter STR save vs restrained + recurring damage; area-effect mechanic not modeled' },
  ...
  'raise-dead': { kind: 'skip', reason: 'resurrection with rejection rights; resurrection primitive needed (revivify partial path)' },
  'rarys-telepathic-bond': { kind: 'skip', reason: 'utility telepathic link, narrative only' },
  'reincarnate': { kind: 'skip', reason: 'resurrection with random species reroll; resurrection + transformation primitive not modeled' },
  ...
  'true-resurrection': { kind: 'skip', reason: 'full resurrection including from any cause of death; resurrection primitive not modeled' },
